...
data class UpdateRouteActivityRequest(
    ...
    @Schema(description = "추가된 루트 활동 이미지")
    var addedActivityImages: List<MultipartFile>?,

    @Schema(description = "삭제된 루트 활동 이미지")
    var deletedActivityImageIds: List<Long>?,
) {
    fun toCommand(activityId: Long): UpdateRouteActivityCommand {
        return UpdateRouteActivityCommand(
            ...
            addedImages = addedActivityImages ?: emptyList(),
            deletedImageIds = deletedActivityImageIds ?: emptyList(),
        )
    }
}
